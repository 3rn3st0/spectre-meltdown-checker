...
check_variant2()
{
	...
	if sys_interface_check "/sys/devices/system/cpu/vulnerabilities/spectre_v2"; then
		...
		# CPUID test
		_info_nol "*   Hardware (CPU microcode) support for mitigation via cpuid: "
		...
		if [ ! -e /dev/cpu/0/cpuid ]; then
			...
			if [ "$opt_verbose" -ge 3 ]; then
				...
				_debug "cpuid: leaf7 is: "$(dd if=/dev/cpu/0/cpuid bs=16 skip=7 iflag=skip_bytes count=1 2>/dev/null | od -x -A n)
				_debug "cpuid: leaf7 edx is: "$(dd if=/dev/cpu/0/cpuid bs=16 skip=7 iflag=skip_bytes count=1 2>/dev/null | dd bs=1 skip=15 count=1 2>/dev/null | od -x -A n)
			fi
			...
			_debug "cpuid: leaf7 high byte of edx is: $edx_hb (decimal)"
			edx_bit26=$(( edx_hb & 8 ))
